...
DEFAULT_SPEC = SCRIPT_DIR / "specs" / "job" / "object_generation.standard.yaml"
    ...
    *,
    ...
    for item in new_results:
        ...
        if key in merged:
            ...
            for field in ("flow_run_id", "submitted", "status", "deployment", "updated_at"):
                if field in item:
                    updated[field] = item[field]
            if "work_queue_name" in item:
                updated["work_queue_name"] = item["work_queue_name"]
            merged[key] = updated
            continue
        merged[key] = item
...
def main() -> int:
    ...
    default_output_path = _default_submitted_manifest_path(
        sweep_id=str(manifest["sweep_id"])
    )
    submitted_manifest = (
