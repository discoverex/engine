...
class _HardVisualVerification:
    ...
    def load(self, handle: ModelHandle) -> None:  # noqa: ARG002
        pass
    def verify(self, composite_image: Path, sigma_levels: list[float]) -> VisualVerification:  # noqa: ARG002
...
class _HardLogicalExtraction:
    ...
    def load(self, handle: ModelHandle) -> None:  # noqa: ARG002
        pass
    def extract(self, composite_image: Path, physical: PhysicalMetadata) -> LogicalStructure:  # noqa: ARG002
...
# 표준 (dummy) 시나리오
_PERC_STD = (_W.perception_sigma * (1.0 / 4.0) + _W.perception_drr * (1.0 - 0.55)) / _P_DENOM  # 0.35
_LOGI_STD = (_W.logical_hop * (2.0 / 4.0) + _W.logical_degree * 1.0 ** 2) / _L_DENOM            # 0.725
...
class TestE2EFullPipelineDummy:
    ...
    def test_returns_verification_bundle(self, orch: ValidatorOrchestrator, composite: Path, tmp_path: Path) -> None:
        layers = [tmp_path / f"obj_{i}.png" for i in range(2)]
        for p in layers:
            _make_png(p)
        assert isinstance(orch.run(composite_image=composite, object_layers=layers), VerificationBundle)

    def test_two_layers_perception_score(self, orch: ValidatorOrchestrator, composite: Path, tmp_path: Path) -> None:
        layers = [tmp_path / f"obj_{i}.png" for i in range(2)]
        for p in layers:
            _make_png(p)
    ...
    def test_two_layers_logical_score(self, orch: ValidatorOrchestrator, composite: Path, tmp_path: Path) -> None:
        layers = [tmp_path / f"obj_{i}.png" for i in range(2)]
        for p in layers:
            _make_png(p)
        bundle = orch.run(composite_image=composite, object_layers=layers)
    ...
    def test_two_layers_total_score(self, orch: ValidatorOrchestrator, composite: Path, tmp_path: Path) -> None:
        layers = [tmp_path / f"obj_{i}.png" for i in range(2)]
        for p in layers:
            _make_png(p)
        bundle = orch.run(composite_image=composite, object_layers=layers)
    ...
    def test_two_layers_passes_threshold(self, orch: ValidatorOrchestrator, composite: Path, tmp_path: Path) -> None:
        layers = [tmp_path / f"obj_{i}.png" for i in range(2)]
        for p in layers:
            _make_png(p)
        bundle = orch.run(composite_image=composite, object_layers=layers)
    ...
    def test_two_layers_answer_obj_count(self, orch: ValidatorOrchestrator, composite: Path, tmp_path: Path) -> None:
        layers = [tmp_path / f"obj_{i}.png" for i in range(2)]
        for p in layers:
            _make_png(p)
        bundle = orch.run(composite_image=composite, object_layers=layers)
    ...
    def test_two_layers_scene_difficulty_signal(self, orch: ValidatorOrchestrator, composite: Path, tmp_path: Path) -> None:
        expected_d = compute_difficulty({
            "occlusion_ratio": 0.45, "sigma_threshold": 4.0, "hop": 2,
            "diameter": 4.0, "degree_norm": 1.0, "detail_retention_rate": 0.55,
        })
        layers = [tmp_path / f"obj_{i}.png" for i in range(2)]
        for p in layers:
            _make_png(p)
        bundle = orch.run(composite_image=composite, object_layers=layers)
        assert bundle.logical.signals["scene_difficulty"] == pytest.approx(expected_d, rel=1e-4)

    def test_required_signal_keys_present(self, orch: ValidatorOrchestrator, composite: Path, tmp_path: Path) -> None:
        layers = [tmp_path / f"obj_{i}.png" for i in range(2)]
        for p in layers:
            _make_png(p)
        bundle = orch.run(composite_image=composite, object_layers=layers)
        ...
        for key in ("answer_obj_count", "scene_difficulty", "degree_map", "hop_map", "diameter"):
            assert key in bundle.logical.signals

    def test_one_layer_total_score(self, orch: ValidatorOrchestrator, composite: Path, tmp_path: Path) -> None:
        """
    ...
    def test_one_layer_answer_obj_count(self, orch: ValidatorOrchestrator, composite: Path, tmp_path: Path) -> None:
        layer = tmp_path / "obj_0.png"
        _make_png(layer)
        bundle = orch.run(composite_image=composite, object_layers=[layer])
        assert bundle.logical.signals["answer_obj_count"] == 1

    def test_no_layers_uses_default_objs(self, orch: ValidatorOrchestrator, composite: Path) -> None:
        bundle = orch.run(composite_image=composite, object_layers=[])
        assert bundle.final.total_score == pytest.approx(_TOT_STD, rel=1e-4)
        assert bundle.final.pass_ is True

    def test_pass_field_is_bool(self, orch: ValidatorOrchestrator, composite: Path) -> None:
        assert isinstance(orch.run(composite_image=composite, object_layers=[]).final.pass_, bool)

    def test_total_score_within_unit_range(self, orch: ValidatorOrchestrator, composite: Path) -> None:
        bundle = orch.run(composite_image=composite, object_layers=[])
    ...
    def test_custom_weights_change_score(self, composite: Path, tmp_path: Path) -> None:
        ...
        biased_orch = _make_orchestrator(
            scoring_weights=ScoringWeights(
                perception_sigma=0.90, perception_drr=0.10,
                logical_hop=0.50, logical_degree=0.50,
                total_perception=0.45, total_logical=0.55,
            )
        )
        s1 = default_orch.run(composite_image=composite, object_layers=layers).final.total_score
        s2 = biased_orch.run(composite_image=composite, object_layers=layers).final.total_score
...
class TestE2EHardScenarioPass:
    ...
    def test_hard_scenario_max_total_score(self, orch: ValidatorOrchestrator, composite: Path, tmp_path: Path) -> None:
        layers = [tmp_path / f"obj_{i}.png" for i in range(2)]
        for p in layers:
            _make_png(p)
        assert orch.run(composite_image=composite, object_layers=layers).final.total_score == pytest.approx(_TOT_HARD, rel=1e-4)

    def test_hard_scenario_passes(self, orch: ValidatorOrchestrator, composite: Path, tmp_path: Path) -> None:
        layers = [tmp_path / f"obj_{i}.png" for i in range(2)]
        for p in layers:
            _make_png(p)
        bundle = orch.run(composite_image=composite, object_layers=layers)
    ...
    def test_hard_scenario_perception_and_logical(self, orch: ValidatorOrchestrator, composite: Path, tmp_path: Path) -> None:
        layers = [tmp_path / f"obj_{i}.png" for i in range(2)]
        for p in layers:
            _make_png(p)
        bundle = orch.run(composite_image=composite, object_layers=layers)
...
class TestE2EPhase4Chain:
    ...
    def test_resolve_and_score_standard_metrics(self) -> None:
        metrics = {
            "occlusion_ratio": 0.45, "sigma_threshold": 4.0,
            "degree": 3, "degree_norm": 1.0, "z_depth_hop": 2,
            "neighbor_count": 3, "hop": 2, "diameter": 4.0,
    ...
    def test_resolve_fails_below_two_conditions(self) -> None:
        assert resolve_answer({"occlusion_ratio": 0.1, "sigma_threshold": 8.0,
                               "degree": 0, "z_depth_hop": 0, "neighbor_count": 0}) is False

    def test_difficulty_and_scene_difficulty_chain(self) -> None:
        obj = {"occlusion_ratio": 0.45, "sigma_threshold": 4.0, "hop": 2,
               "diameter": 4.0, "degree_norm": 1.0, "detail_retention_rate": 0.55}
    ...
    def test_hard_metrics_full_chain(self) -> None:
        hard = {"occlusion_ratio": 0.5, "sigma_threshold": 1.0, "degree": 4,
                "degree_norm": 1.0, "z_depth_hop": 3, "neighbor_count": 4,
                "hop": 4, "diameter": 4.0, "detail_retention_rate": 0.0}
    ...
    def test_easy_metrics_full_chain(self) -> None:
        easy = {"occlusion_ratio": 0.0, "sigma_threshold": 16.0, "degree": 0,
                "degree_norm": 0.0, "z_depth_hop": 0, "neighbor_count": 0,
                "hop": 0, "diameter": 1.0, "detail_retention_rate": 0.99}
    ...
    def test_total_score_is_weighted_sum_of_sub_scores(self) -> None:
        metrics = {"sigma_threshold": 2.0, "detail_retention_rate": 0.4,
                   "hop": 3, "diameter": 4.0, "degree_norm": 0.6}
        w = ScoringWeights()
        perc, logi, total = integrate_verification_v2(metrics)
        assert total == pytest.approx(perc * w.total_perception + logi * w.total_logical, rel=1e-6)
    ...
    def test_custom_weights_alter_score(self) -> None:
        metrics = {"sigma_threshold": 4.0, "detail_retention_rate": 0.55,
                   "hop": 2, "diameter": 4.0, "degree_norm": 1.0}
        _, _, total_default = integrate_verification_v2(metrics)
        w_custom = ScoringWeights(perception_sigma=0.90, perception_drr=0.10,
                                  logical_hop=0.90, logical_degree=0.10)
...
def test_phase1_real_mobilesam_loads() -> None:
    from discoverex.adapters.outbound.models.hf_mobilesam import HFMobileSAMExtraction  # noqa: F401
...
def test_phase2_real_moondream2_loads() -> None:
    from discoverex.adapters.outbound.models.hf_moondream2 import HFMoondream2Extraction  # noqa: F401
    raise AssertionError("이 테스트는 skip 되어야 함")

@pytest.mark.skip(reason="YOLO+CLIP: transformers 미설치")
def test_phase3_real_yolo_clip_loads() -> None:
    from discoverex.adapters.outbound.models.hf_yolo_clip import HFYoloCLIPVerification  # noqa: F401
    raise AssertionError("이 테스트는 skip 되어야 함")
