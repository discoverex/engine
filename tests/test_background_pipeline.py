...
def test_apply_background_hires_fix_updates_background_dimensions(
    tmp_path: Path,
) -> None:
    source = tmp_path / "background.png"
    ...
    class _FakeBackgroundModel:
        def predict(self, _handle, request):  # type: ignore[no-untyped-def]
            ...
            Path(request.params["output_path"]).parent.mkdir(
    ...
    updated = apply_background_hires_fix_if_needed(
        ...
        context=context,
        scene_dir=tmp_path,
        upscaler_handle=ModelHandle(name="bg-upscale", version="test", runtime="fake"),
